...
DestDir=$FCNC_DIR/FCNCFigures/tthML/$1
mkdir -p $DestDir
for sig in ${signal[@]}
do
	#echo "cp $sig/combined_BDT_test/Limits.pdf $DestDir/Limit/StatOnly/$sig.pdf"
	#cp $sig/combined_BDTG_test/Limits.pdf $DestDir/Limit/StatOnly/$sig.pdf
	for reg in ${region[@]}
	do
		echo "cp $sig/combined_BDTG_test/Plots/$reg.pdf $DestDir/Limit/${sig}_$reg.pdf"
		cp $sig/combined_BDTG_test/Plots/$reg.pdf $DestDir/Limit/${sig}_$reg.pdf
		echo "cp $sig/combined_BDTG_test/Plots/${reg}_postFit.pdf $DestDir/Limit/${sig}_${reg}_postFit.pdf"
		cp $sig/combined_BDTG_test/Plots/${reg}_postFit.pdf $DestDir/Limit/${sig}_${reg}_postFit.pdf
	done
done
...
cp combined_BDTG_test/Tables/Yields*.tex $FCNC_DIR/FCNCTables/tthML/yield
